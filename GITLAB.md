...
# 버전 고정 (권장 — 태그)
pip install "docstruct @ git+https://183.96.152.133/mjseo/docstruct.git@v0.1.29"

# 포맷별 의존성 함께
pip install "docstruct[hwp] @ git+https://183.96.152.133/mjseo/docstruct.git@v0.1.29"
pip install "docstruct[all] @ git+https://183.96.152.133/mjseo/docstruct.git@v0.1.29"

# SSH (키가 등록되어 있을 때)
pip install "docstruct @ git+ssh://git@183.96.152.133/mjseo/docstruct.git@v0.1.29"
```

`requirements.txt` 에 넣을 때도 같은 문자열을 씁니다.

```
docstruct[hwp] @ git+https://183.96.152.133/mjseo/docstruct.git@v0.1.29
```
...
```bash
pip install -U "docstruct @ git+https://183.96.152.133/mjseo/docstruct.git@v0.1.29"
```
