...
def document_markdown(doc: PageDocument) -> str:
    """본문을 markdown 문자열로 만든다.

    입력: doc — PageDocument
    출력: markdown 문자열 (표·이미지 placeholder 가 펼쳐진 상태)
    비고:
        `write_markdown` 이 파일로 쓰는 것과 같은 내용이다. 서비스처럼
        **문자열로 돌려줘야 하는 자리**를 위해 나눠 두었다 — 그런 자리가
        컨버터의 원재료(`hwpxtree.to_markdown` 등)를 그대로 쓰다가
        그림·정규화가 통째로 빠지는 일이 있었다.
    """
    return _markdown_body(doc)


def _markdown_body(doc: PageDocument) -> str:
    """쪽마다 펼쳐 이어 붙인다."""
...
def write_markdown(doc: PageDocument, path: str | Path) -> Path:
    ...
    target.write_text(document_markdown(doc), encoding="utf-8")
    return target
...
def write_pipeline_report(doc: PageDocument, path: str | Path) -> Path:
    ...
    if notes or doc.failed_pages:
        ...
        if doc.failed_pages:
            ...
            for reason in (doc.failure_reasons or [])[:3]:
...
def write_layout_report(doc: PageDocument, path: str | Path) -> Path:
    ...
    """
    from docstruct.output.layout import LABEL_KO, OUTCOME_KO, label_counts, overlapping_pairs
