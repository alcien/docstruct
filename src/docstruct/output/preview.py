...
def content_for_markdown(page: PageContent) -> str:
    ...
    # 0.4.89 블록(`<image N> … </image N>`)을 먼저 라벨로 바꾸고,
    # 남은 옛 표식은 아래 정규식이 맡는다.
    content = _image_blocks_to_labels(content, page)
    return _IMG_PLACEHOLDER_RE.sub(_img_repl, content)


def _image_blocks_to_labels(content: str, page: PageContent) -> str:
    """`<image N>` 블록을 사람이 읽을 라벨로 바꾼다.

    입력: content — 본문, page — 그 쪽
    출력: 라벨로 바뀐 본문
    """
    from docstruct.images.tags import IMAGE_BLOCK_RE, parse_image_block

    by_num = {getattr(i, "image_num", None): i for i in page.images}

    def _repl(m: re.Match[str]) -> str:
        num = int(m.group(1))
        info = by_num.get(num)
        got = parse_image_block(content, num) or {}
        label = f"**🖼 {info.id if info else f'image_{num}'}**"
        desc = got.get("description") or (info.description if info else "")
        read = got.get("read") or ""
        head = f"{label} — {desc}" if desc else label
        return f"\n{head}\n\n{read}\n" if read else f"\n{head}\n"

    return IMAGE_BLOCK_RE.sub(_repl, content)
...
def table_markdown_for_view(table) -> str:
    """표시용 markdown — 이어지는 표면 물려받은 머리를 붙여 준다.

    입력: table — TableInfo
    출력: markdown 문자열 (원본은 바꾸지 않는다)
    비고:
        지면은 머리를 첫 쪽에만 인쇄한다. 그대로 보면 데이터부터 시작해
        어느 열이 무엇인지 알 수 없다 — 실측(조달청 p70): 17열 표에서
        "재정사업 평가명" 이 몇 번째 열인지 볼 수 없었다.

        **원본에는 넣지 않는다** (0.3.9 결정). 열 수가 쪽마다 13~17 로
        달라 저장하면 되돌릴 수 없는 정렬 사고가 난다. 보여 줄 때만
        붙인다.
    """
    body = getattr(table, "markdown", "") or ""
    header = getattr(table, "inherited_header", None)
    if not header:
        return body
    from docstruct.tables.continued import with_header

    return with_header(body, header) or body
...
def table_detail_html(page: PageContent, table: TableInfo) -> str:
    ...
    if not table.was_filled:
        return header + _pre(table_markdown_for_view(table))
    ...
    return header + (
        ...
        f"{_pre(table_markdown_for_view(table), bg='#f0fdf4')}</div>"
        "</div>"
    )
...
def layout_html(page: PageContent) -> str:
    ...
    """
    from docstruct.output.layout import OUTCOME_KO, overlapping_pairs
...
#: 즉석 렌더한 지면 이미지를 모아 두는 곳. 슬라이더를 움직일 때마다 다시
#: 그리면 느리므로 한 번 그린 쪽은 다시 쓴다.
_RENDER_CACHE: dict[tuple[str, int, float], str] = {}


def page_image(page, pdf_path=None, *, scale: float = 2.0) -> str | None:
    """이 쪽의 지면 이미지 경로 — 없으면 원본 PDF 에서 즉석으로 그린다.

    입력:
        page      PageContent
        pdf_path  원본 PDF 경로 (없으면 즉석 렌더를 못 한다)
        scale     렌더 배율 (1.0 = 72dpi)
    출력: PNG 경로. 그릴 수 없으면 None
    비고:
        파이프라인은 **표가 있는 쪽만** 렌더한다 — 렌더의 원래 용도가 표
        재추출의 시각 근거이기 때문이다(전 쪽을 그리면 155쪽 문서에서
        50초쯤 헛돈다). 그래서 표지·목차처럼 표가 없는 쪽은
        `page_image_path` 가 비어 있고, 지금까지 미리보기는 **아무 말 없이
        그 쪽을 건너뛰었다** — 실측(행안부): 429쪽 중 이미지가 있는 쪽은
        289쪽뿐이라 1~6쪽을 열면 아무것도 보이지 않았다.

        판독 결과를 지면과 대조하려면 **모든 쪽**이 보여야 하므로, 원본
        PDF 가 있으면 그 자리에서 그려 캐시에 둔다.
    """
    saved = getattr(page, "page_image_path", None)
    if saved and Path(saved).is_file():
        return saved

    page_no = getattr(page, "page_no", None)
    if not pdf_path or not isinstance(page_no, int):
        return None
    source = Path(pdf_path)
    if not source.is_file() or source.suffix.lower() != ".pdf":
        return None

    key = (str(source), page_no, scale)
    cached = _RENDER_CACHE.get(key)
    if cached and Path(cached).is_file():
        return cached
    try:
        from docstruct.images.page_render import render_page

        out_dir = Path(tempfile.gettempdir()) / "docstruct-preview"
        out_dir.mkdir(parents=True, exist_ok=True)
        rendered = render_page(source, page_no, out_dir, scale=scale)
    except Exception as exc:                     # noqa: BLE001 - 미리보기가 멈추면 안 된다
        _log.debug("%s쪽 즉석 렌더 실패: %s", page_no, exc)
        return None
    if rendered:
...
def show_page(
    ...
    *,
    ...
    입력:
        page            PageContent
        pdf_path        원본 PDF (렌더되지 않은 쪽을 즉석으로 그린다)
    ...
    if show_image:
        ...
        if shot:
            try:
                ...
                _display(Image(filename=shot, width=image_width))
        else:
            # **조용히 넘어가지 않는다.** 이전에는 아무 표시가 없어
            # "이미지가 안 보인다" 로만 보였고, 원인(표 없는 쪽은 렌더
            # 대상이 아니다)을 알 길이 없었다.
            _html(
                '<p style="color:#94a3b8;font-size:12px;">'
                "지면 이미지가 없습니다 — 표가 없는 쪽은 렌더 대상이 "
                "아닙니다. 원본 PDF 경로를 <code>pdf_path=</code> 로 주면 "
                "그 자리에서 그립니다.</p>"
            )
...
def show_pages(doc: PageDocument, *, limit: int | None = None, **kwargs) -> None:
    ...
    입력: doc — PageDocument, limit — 앞에서 몇 페이지만 (None=전부),
          kwargs — show_page 로 전달 (pdf_path 포함)
    출력: 없음
    """
