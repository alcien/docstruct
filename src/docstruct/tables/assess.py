...
### 빈 칸을 데이터 손실로 오해하지 마세요 (중요)
markdown 표는 병합 셀(rowspan/colspan)을 표현하지 못합니다. 원본에서 여러 칸에 걸친 셀은 **왼쪽 위 칸에만 값을 넣고 나머지는 빈 칸으로** 옮겼습니다. 값을 복제하면 같은 내용이 검색에 여러 번 걸리기 때문입니다.

따라서 아래는 정상이며 insufficient가 아닙니다.
- 어떤 행의 오른쪽 칸들이 비어 있고, 바로 위 행의 같은 열에 값이 있는 경우 → 세로 병합
- 어떤 행의 첫 칸에만 값이 있고 나머지가 비어 있는 경우 → 가로 병합(제목 행 등)
- 원본 자체가 빈 칸인 경우 (예: 아직 값이 없는 계획 항목)

표 **전체**가 비었거나, 병합으로 설명되지 않는 자리에서 값이 사라졌을 때만 insufficient로 판단하세요.
