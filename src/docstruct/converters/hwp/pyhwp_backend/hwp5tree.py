...
from docstruct.converters.hwp.marks import PAGE_BREAK  # noqa: F401
...
#: 구역 수를 알 수 없을 때 몇 개까지 더듬어 볼지 (0.4.86).
#: 탐색 한 번이 그 구역을 **실제로 읽으므로** 상한이 곧 최악의 낭비다.
_MAX_SECTION_PROBE = max(1, int(os.environ.get("DOCSTRUCT_HWP_MAX_SECTIONS", "64")))
...
def to_markdown(hwp_path: str) -> str:
    ...
    with _quiet_warnings() as counter:
        ...
        try:
            ...
            for index in range(_section_count(hwp)):
                ...
                section_times.append(time.perf_counter() - _t_sec)
        finally:
    ...
    # **어디서 시간이 갔는지 남긴다** (0.4.86). 예전에는 파이프라인이
    # `추출 (HWP 파싱) 1663초 100%` 한 줄만 찍어 그 안이 통째로 깜깜했다 —
    # 구역이 많아서인지 한 구역이 무거워서인지 가릴 수 없었다.
    elapsed = time.perf_counter() - _t0
    if section_times:
        _log.info(
            "hwp5-tree 파싱 %.1f초 — 구역 %d개 (가장 느린 구역 %.1f초) · 블록 %d개",
            elapsed, len(section_times), max(section_times), len(parts),
        )
    else:
        _log.info("hwp5-tree 파싱 %.1f초 — 읽은 구역 없음", elapsed)
    ...
    norm = time.perf_counter() - _t_norm
    if norm > 1.0:
        _log.info("한국어 정규화 %.1f초 (%d블록 · %d자)", norm, len(kept), len(out))
    return out
...
def _section_count(hwp) -> int:
    ...
    if isinstance(count, int) and count > 0:
        return count
    # **탐색은 값싸지 않다** (0.4.86). `section(i)` 는 그 구역 스트림을 실제로
    # 읽는다. 예전 상한 1000 은 "끝나면 예외가 난다" 를 전제한 것인데, pyhwp
    # 버전에 따라 범위를 넘겨도 예외 없이 빈 것을 돌려준다 — 그러면 여기서
    # 1000번 읽고 아래 본 루프에서 또 1000번 읽는다. 빈 구역이 나오면 끝으로
    # 본다. 진짜로 64구역이 넘는 문서라면 DOCSTRUCT_HWP_MAX_SECTIONS 로 올린다.
    found = 0
    while found < _MAX_SECTION_PROBE:
        try:
            section = hwp.bodytext.section(found)
        except Exception:                        # noqa: BLE001 - 더 없으면 끝
            break
        if section is None:
            break
        found += 1
    if found >= _MAX_SECTION_PROBE:
        _log.warning(
            "구역 수를 %d개에서 끊었습니다 — pyhwp 가 끝을 알리지 않는 "
            "문서입니다. 더 필요하면 DOCSTRUCT_HWP_MAX_SECTIONS 를 올리세요.",
            _MAX_SECTION_PROBE,
        )
    return found
...
def _render_table(table: _Table) -> str:
    ...
    for cell in table.cells:
        ...
        if not mark_merges or cell.rowspan <= 1:
            continue
        # 세로로 이어지는 칸을 채운다 (맨 윗행은 값 그대로).
        # 무엇으로 채울지는 `converters.common.table` 한 곳이 정한다 (0.5.6).
        filler = merge_continuation(anchor)
        for row in range(cell.row + 1, min(cell.row + cell.rowspan, rows)):
            grid[row][cell.col] = filler
