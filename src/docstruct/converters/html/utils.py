...
def normalize_line(text: str) -> str:
    """연속 공백·개행을 한 줄 텍스트로 정리합니다."""
    return " ".join(text.split())


def tag_text(tag: "Tag") -> str:
    """
    hwp5html span 경계의 공백을 보존하면서 텍스트를 추출합니다.

    get_text(separator=' ')는 한글 단어 중간에 공백을 넣고,
    get_text()는 span 경계 공백을 제거하므로, 텍스트 노드를 직접 이어 붙입니다.
    """
    parts: list[str] = []
...
def bullet_to_md(text: str) -> str:
    """HWP 스타일 불릿 단락을 GFM 목록 항목으로 변환합니다."""
