...
호출부:
    ...
    docstruct.experiments.text.scan_scale_ab (safe_file_stem 만)
출력:
...
def safe_file_stem(name: str) -> str:
    """**파일 안쪽 이름**으로 쓸 수 있게 정리한다 (확장자를 뗀다).

    입력: name — 원본 파일명
    출력: 경로 구분자·특수문자를 제거한 stem (`성과계획서.hwpx` → `성과계획서`)
    비고:
        `조달청_image_1.png` 처럼 **폴더 안 파일 이름**을 만들 때 쓴다.
        여기에 확장자가 붙으면 `조달청.hwpx_image_1.png` 이 되어 읽기
        나쁘다.

        **산출 폴더 이름에는 쓰지 않는다.** 폴더는 확장자를 포함해야
        `성과계획서.hwpx/` 와 `성과계획서.pdf/` 가 갈린다 —
