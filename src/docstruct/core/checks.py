...
호출부:
    ...
    docstruct.cli --check
    docstruct.output.colab (check_llm_reachable 재노출)
출력:
...
def _hwp_via_hwpx() -> tuple[bool, str]:
    """HWP→HWPX 변환이 가능한가 (0.5.3).

    입력: 없음
    출력: (가능한가, 어느 변환기인지)
    비고:
        이 경로가 **가장 좋은 결과**를 낸다 — HWPX 로 바꾸면 `cells` 가
        생겨 격자 검사·오염 검사가 HWP 에서도 돈다.
    """
    try:
        from docstruct.converters.hwpx import convert
    except ImportError:
        return False, ""
    if convert.python_backend() is not None:
        return True, "hwp2hwpx (파이썬)"
    if convert.is_available():
        return True, "hwp2hwpx (jar)"
    return False, ""


def _hwp_note(pyhwp: bool, hwp5html: bool) -> str:
    """HWP 경로 안내 한 줄.

    입력: pyhwp — hwp5 설치 여부, hwp5html — 실행파일 탐지 여부
    출력: 사람이 읽을 문장
    비고:
        **라이선스 사실을 여기서 알린다** (0.5.3). `hwp2hwpx` 파이썬
        변환기는 `pyhwp`(AGPL)에 기댄다. `converters/hwp/pyhwp_backend/` 를
        떼어낸 배포에서 이것을 설치하면 pyhwp 가 **런타임으로 되돌아온다** —
        모르고 하면 안 되는 선택이라 점검 화면에서 말한다.
    """
    via, which = _hwp_via_hwpx()
    parts = []
    if via:
        parts.append(f"{which} 로 HWPX 변환 — 표 구조(cells)까지 살아납니다")
        if "파이썬" in which and _installed("hwp5"):
            parts.append("주의: 이 변환기는 pyhwp(AGPL)에 기댑니다")
    if pyhwp and hwp5html:
        parts.append("pyhwp + hwp5html")
    elif not via:
        parts.append("pyhwp 미설치/hwp5html 미탐지 — olefile 폴백(표 구조 손실). "
                     "`hwp2hwpx` 를 설치하면 표가 살아납니다")
    return " · ".join(parts)
...
def environment() -> list[dict[str, Any]]:
    ...
    checks: list[dict[str, Any]] = [
        ...
        {
            "item": "HWP 파싱",
            "ok": bool(_hwp_via_hwpx()[0] or (pyhwp and hwp5html)),
            "note": _hwp_note(pyhwp, hwp5html),
        },
        {
            "item": "HWPX 파싱",
            "ok": hwpx,
