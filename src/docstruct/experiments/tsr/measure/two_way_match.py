...
def run(pages: list[PageContent], *, scale: float = 2.0, **_kwargs) -> int:
    ...
    for page in pages:
        ...
        for table in page.tables:
            ...
            marked += 1
            page.trace.add(
                "experiments.tsr.measure.two_way_match", "매칭 불일치",
...
register(Experiment(
    ...
    status="testing",
    note="두 문서에서 검출률 10~11% 로 일관됐다 (국세청 6/61 · 행안부 35/321). "
         "값이 한 칸에 뭉친 자리를 짚는다 — `1,243 1,796` 처럼. "
         "표시만 하며, HWP 계열은 좌표가 없어 대상 아님. "
         "스캔본은 텍스트 레이어가 없어 건너뛴다.",
    run=run,
))
