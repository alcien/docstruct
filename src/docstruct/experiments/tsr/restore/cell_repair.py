...
def _is_separator_cell(cell: str) -> bool:
    ...
    입력: cell — 셀 문자열 (strip 된 상태)
    출력: 대시·콜론만으로 되어 있고 대시가 하나 이상이면 True
    비고:
        `---` 정확일치로 두면 안 된다. 파이프라인 실물 표는
        `render_md_table` 이 열 폭에 맞춰 늘인 구분선(`------`)을 쓰므로,
        정확일치는 구분선을 데이터 행으로 오인해 **구분선에 빈 칸을 박아
        표 전체를 깨뜨렸다.** `:---:` 정렬 표기도 함께 받는다.
    """
    return bool(cell) and not (set(cell) - set("-:")) and "-" in cell
...
def repair_table(markdown: str, *, row_merge: bool = False) -> tuple[str, int]:
    ...
    for line in lines if row_merge else []:
        ...
        rebuilt.append(line)
    if row_splits:
        # rebuilt 에 모든 행이 이미 들어 있다 — 뭉친 행은 두 행으로,
        # 나머지는 그대로.
        markdown = "\n".join(rebuilt)
        rows = _rows_of(markdown)
    ...
    for position, line in enumerate(lines):
        ...
        if all(not c or _is_separator_cell(c) for c in cells):
            if any(_is_separator_cell(c) for c in cells):   # 대시가 있으면 구분선
                cells.insert(column, "---")
                lines[position] = "|" + "|".join(cells) + "|"
            else:                            # 빈 행이면 칸만 늘린다
                cells.insert(column, "")
...
def run(pages: list[PageContent], **_kwargs) -> int:
    ...
    for page in pages:
        for table in page.tables:
            ...
            fixed += 1
            page.trace.add(
                "experiments.tsr.restore.cell_repair", "뭉친 값 분리",
