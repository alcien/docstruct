...
add_rows(ws, [
    ...
    ["5-2", "└ 그림 저장", "docstruct.images.picture", "save_picture()",
     "PictureItem 이미지를 파일로 저장하고 ImageInfo 생성", "PictureItem", "ImageInfo + PNG", "-", "필수"],
    ["5-3", "└ 레이아웃 기록", "docstruct.output.layout", "collect_layout()",
     "인식 라벨 vs 파이프라인 처리 결과를 대조 기록 (진단용)",
     "DoclingDocument", "list[LayoutItem]", "-", "필수"],
    ["6", "페이지 렌더", "docstruct.images.page_render", "render_page_images()",
    ...
    ["11", "출력", "docstruct.output.report", "write_json / write_markdown / …",
...
add_rows(ws, [
    ...
    ["공통", "6", "docstruct.text.split", "split_document()",
...
add_rows(ws, [
    ...
    ["6", "docstruct.text.split", "split_document()",
...
add_rows(ws, ["계층", "패키지 / 모듈", "역할", "주요 함수·클래스", "의존"], [
    ...
    ["L3", "docstruct.images", "이미지 — 페이지 렌더·그림 저장·인코딩",
